...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	data, err := json.Marshal(order) // data is byte array
	if err != nil {
	...
	// group transaction to ensure all the db transactions are atomic, to avoid partial state
	txn := r.Client.TxPipeline()

	//SetNX = set non-exist, would not overwrite data that already exist instead returning the error
...
func (r *RedisRepo) DeleteByID(ctx context.Context, id uint64) error {
	...
	if err := txn.SRem(ctx, "orders", key).Err(); err != nil {
		txn.Discard()
		return fmt.Errorf("failed to delete from orders set: %w", err)
	}
...
func (r *RedisRepo) Update(ctx context.Context, order model.Order) error {
	...
	// SetXX, only update the existing value
...
type FindAllPage struct {
	Size uint64
	Offset uint64 // can be renamed as cursor or count
}
